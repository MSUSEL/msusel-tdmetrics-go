...
Type `/` followed by `code-task-generator` in the Cursor chat to execute this workflow.

## Parameters
When you execute this command, I will prompt you for the following parameters:
### Required Parameters
- **input**: Task description, file path, or PDD plan path. Can be a simple sentence, paragraph, detailed explanation, or path to a PDD implementation plan.
### Optional Parameters
- **step_number**: For PDD plans only - specific step to process. If not provided, automatically determines the next uncompleted step from the checklist.
- **output_dir**: Directory where the code task file will be created (default: ".agents/tasks/{project_name}")
- **project_name**: Project name for organizing tasks. If processing a PDD plan, will be inferred from the plan path. Otherwise, will be generated from the description with a YYYY-MM-DD date prefix.
...
Run this SOP:
<agent-sop name="code-task-generator">
<content>
# Code Task Generator
