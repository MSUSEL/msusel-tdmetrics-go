...
Type `/` followed by `code-assist` in the Cursor chat to execute this workflow.

## Parameters
When you execute this command, I will prompt you for the following parameters:
### Required Parameters
- **task_description**: A description of the task to be implemented. This can be a detailed specification with requirements and acceptance criteria, or even a rough idea that will be refined during the explore and plan phases
### Optional Parameters
- **additional_context**: Any supplementary information that would help with understanding the implementation context
- **documentation_dir**: The directory where planning documents will be stored (default: ".agents/scratchpad/{project_name}")
- **repo_root**: The root directory of the repository for code implementation (default: current working directory)
- **project_name**: Project name for organizing working files. If processing a code task file, will be inferred from the task path. Otherwise, will be generated from the task description with a YYYY-MM-DD date prefix.
- **task_name**: A short, descriptive name for the implementation task
- **mode**: The interaction mode: - "interactive": Collaboration with user confirmation at each step - "auto": No user interaction after initial setup (default: "auto")
