...
Type `/` followed by `eval` in the Cursor chat to execute this workflow.

## Parameters
When you execute this command, I will prompt you for the following parameters:
### Required Parameters
- **agent_path**: Path to the agent folder to evaluate (e.g., `./chatbot-agent`, `/path/to/my-agent`)
### Optional Parameters
- **evaluation_focus**: Specific aspects to evaluate (e.g., "response quality", "tool calling accuracy")
- **test_case_count**: Number of test cases to generate (default: 3)
- **output_format**: Format for test data output (default: "jsonl")
