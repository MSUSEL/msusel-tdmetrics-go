...
func Test_ConstructInitInstance(t *testing.T) {
	tt := parseDecl(t, `newFoo`,
		`type Foo[T string|int] struct{ value T }`,
		`func newFoo[T string|int](value T) *Foo[T] {`,
		`  return &Foo[T]{ value: value }`,
		`}`)
	tt.checkProj(
		`{`,
		`  language: go,`,
		...
		`      lineCount:  3,`,
		`      reads:  [ tempReference1 ],`,
		`      writes: [ selection1, tempReference1 ]`,
		`    }`,
		`  ],`,
		`  packages: [`,
		`    { name: test, path: test }`,
		`  ],`,
		`  selections: [`,
		`    { name: value, origin: tempReference1 }`,
		`  ],`,
		`  tempReferences: [`,
		`    {`,
		`      name: Foo, packagePath: test,`,
...
func Test_DeepConstructInit(t *testing.T) {
	...
	tt.checkProj(
		...
		`      indents:    8,`,
		`      lineCount:  4,`,
		`      reads:  [ tempReference1, tempReference2 ],`,
		`      writes: [ selection1, selection2, selection3, selection4, tempReference1, tempReference2 ]`,
		`    }`,
		`  ],`,
		`  packages: [`,
		`    { name: test, path: test }`,
...
func Test_InstanceCalls(t *testing.T) {
	tt := parseDecl(t, `baz`,
		`type foo[T string|int|uint] struct { v T }`,
		`func (f foo[T]) bar(v T) T { return f.v + v }`,
		`func newFoo[T string|int](v T) foo[T] { return foo[T]{v: v} }`,
		`func baz() {`,
		`   c := newFoo("Hello")`,
		`   c.bar("World")`,
		`}`)
	tt.checkProj(
		...
		`      indents:    6,`,
		`      lineCount:  4,`,
		`      invokes: [ selection1, tempDeclRef1 ],`,
		`      reads:   [ tempReference1 ],`,
		`      writes:  [ tempReference1 ]`,
		`    }`,
		`  ],`,
		...
		`  ],`,
		`  selections: [`,
		`    { name: bar, origin: tempReference1 }`,
		`  ],`,
		`  tempDeclRefs: [`,
		`    {`,
		`      name: newFoo, packagePath: test,`,
