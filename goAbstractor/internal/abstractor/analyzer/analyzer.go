...
func Analyze(info *types.Info, proj constructs.Project, curPkg constructs.Package, conv converter.Converter, node ast.Node) constructs.Metrics {
	var (
		loc    = proj.Locs().NewLoc(node.Pos())
		cmplx  = complexity.Calculate(node, proj.Locs().FileSet())
		acc    = accessor.Calculate(info, node)
		usages = usages.Calculate(info, proj, curPkg, conv, node)
	)
