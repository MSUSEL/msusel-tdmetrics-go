...
1. [ ] **`CtConstructor.getType()` returns a raw reference to the declaring type.**
...
1. [ ] **Synthetic references built by `parameterizedRef` have no AST parent.**
...
1. [ ] **`CtTypeParameter.getTypeErasure()` only returns the first bound** for multi-bounded
...
1. [ ] **`CtWildcardReference.getBoundingType()` has the same single-bound limitation**
...
1. [ ] **`getAllMethods()` pulls inherited methods from JDK shadow super-interfaces.**
...
1. [ ] **Package imports are not derived yet.** Already noted in `Abstractor.performAbstraction`'s
  comment; agree it's still pending and ties into the planned Resolver pipeline.

1. [ ] **`Validator` runs after `crossConnectConstructs`, but `crossConnectConstructs`**
