...
public class JsonFormat {
    ...
    static final String constKeyValueSep = ":";
    static final String constElemSep     = ",";
    static final String constArrayOpen   = "[";
    static final String constArrayClose  = "]";
    static final String constObjectOpen  = "{";
    static final String constObjectClose = "}";
    static final String constNull        = "null";
    static final String constTrue        = "true";
    static final String constFalse       = "false";
    static final String constUnknown     = "unknown_node";

    static private boolean needsEscape(String text) {
        if (text.equalsIgnoreCase(constNull) ||
            text.equalsIgnoreCase(constTrue) ||
            text.equalsIgnoreCase(constFalse))
            return true;
    ...
    private enum Style {
        ...
        public String firstSep(boolean canInline, String indent2) {
            ...
            if (canInline) return constSingle;
            return constNewLine + indent2;
        }

        public String restSep(boolean canInline, String indent2) {
            if (this == minimize) return constElemSep;
            if (this == inline) return constElemSep + constSingle;
            if (canInline) return constElemSep + constSingle;
            return constElemSep + constNewLine + indent2;
        }

        public String tailSep(boolean inline, String indent) {
            if (this == minimize) return constEmpty;
            if (inline) return constSingle;
            if (this == relaxed) return constElemSep + constNewLine + indent;
            return constNewLine + indent;
        }

        public String keyValSep() {
            if (this == minimize) return constKeyValueSep;
            return constKeyValueSep + constSingle;
        }
    ...
    private void format(PrintStream sb, JsonNode j, String indent, int depth) {
        ...
        else sb.append(constUnknown);
    }

    private void format(PrintStream sb, JsonValue j, String indent, int depth) {
        if (j.isString()) sb.append(this.style.getString(j.asString()));
        else sb.append(j.asString());
    }

    private void format(PrintStream sb, JsonArray j, String indent, int depth) {
        if (j.isEmpty()) {
            sb.append(constArrayOpen + this.style.emptySep() + constArrayClose);
            return;
        }
        ...
        sb.append(tailSep);
        sb.append(constArrayClose);
    }

    private void format(PrintStream sb, JsonObject j, String indent, int depth) {
        if (j.isEmpty()) {
            sb.append(constObjectOpen + this.style.emptySep() + constObjectClose);
            return;
        }
